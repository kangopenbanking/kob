...
const guideSections: GuideSection[] = [
  ...
  },
  {
    ...
    description: 'Add team members, assign roles, and control exactly what each person can see and do in your travel portal.',
    steps: [
      { title: 'How Staff Access Works', detail: 'When you add a staff member, the system creates a secure login account for them. You set their email, password, phone number, and a 6-digit PIN. Staff can log in two ways: (1) Email + Password, or (2) Phone Number + 6-digit PIN. Both methods take them to the dedicated Staff Portal at /staff-login where they only see sections their permissions allow.' },
      { title: 'The Staff Login Page', detail: 'Staff use a dedicated login page at /staff-login (not the main login). Click "Copy Login Link" on the Staff Roles page and share the link with your team. The login page has two tabs: "Email" for email+password login, and "Phone + PIN" for quick PIN-based access. After signing in, staff are redirected to the Travel Services portal with their permitted sections only.' },
      { title: 'Open Staff Roles', detail: 'From Travel Services, click "Staff Roles". This opens the Staff & Role Access management panel showing all your team members, their roles, active permissions, and status.' },
      { title: 'Add a Staff Member', detail: 'Click "Add Staff". Fill in: Full Name, Email (required — used for account creation), Phone Number (optional — needed for PIN login), Password (you set this — min 6 characters), and 6-Digit PIN (you set this — used for quick Phone+PIN login). Then choose a role preset and click "Create Staff Account".' },
      { title: 'Setting the Password', detail: 'You (the merchant) set the initial password for each staff member. Share it with them securely (in person, phone call, or encrypted message). Staff use this password with their email to log in. Passwords must be at least 6 characters.' },
      { title: 'Setting the 6-Digit PIN', detail: 'You set a 6-digit numeric PIN for each staff member. This PIN is used with their phone number for quick sign-in (ideal for counter or boarding staff who need fast access). Enter the PIN using the on-screen keypad in the Add Staff dialog. Share the PIN with your staff securely.' },
      { title: 'Customize Permissions', detail: 'After selecting a role preset, fine-tune individual permissions using toggle switches. 9 permission areas: Services, Routes, Seating, Timetable, Bookings, Discounts, Scanner, Notifications, and Reports. Toggle each ON or OFF as needed.' },
      { title: 'Edit a Staff Member', detail: 'Click the pencil icon to change their name, phone, role, or permissions. Note: email and password cannot be changed after creation (for security). Changes to permissions take effect immediately.' },
      { title: 'Activate / Deactivate Staff', detail: 'Use the toggle switch in the Status column to temporarily disable a staff member without deleting them. Deactivated staff cannot log in. Reactivate them anytime by toggling back on. This is ideal for seasonal workers.' },
      { title: 'Remove Staff', detail: 'Click the trash icon to permanently remove a staff member. This revokes all their access immediately. You will be asked to confirm before deletion.' },
      { title: 'Role Presets Explained', detail: 'Admin: Full access to everything. Manager: All operations including services, routes, seating, schedules, bookings, discounts, scanner, notifications, reports. Booking Agent: Bookings and scanner only — ideal for counter staff. Scanner: Ticket scanning only — ideal for boarding agents. Viewer: No active permissions — starting point for custom roles.' },
    ],
    tips: [
      'Share the staff login link (/staff-login) with your team — it\'s their dedicated entry point.',
      'Use Phone + PIN login for counter and boarding staff who need fast access without typing passwords.',
      'Use "Booking Agent" for counter staff — they get bookings and scanner access only.',
      'Use "Scanner" for boarding agents who only need to validate tickets at the gate.',
      'Admin role gives full access — only assign to trusted senior staff.',
      'Always share passwords and PINs securely — never via public channels.',
      'Deactivating a staff member is better than deleting if they might return (e.g., seasonal workers).',
      'Permissions update in real-time — staff see changes on their next page load.',
      'Each staff member must have their own unique email — do not share accounts.',
    ],
  },
  {
