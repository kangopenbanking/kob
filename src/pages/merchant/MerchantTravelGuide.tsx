...
const guideSections: GuideSection[] = [
  ...
  },
  {
    ...
    steps: [
      { title: 'How Staff Access Works', detail: 'When you add a staff member with their email, they receive an invitation to create an account (or log in if they already have one). Once logged in, they are automatically directed to the Merchant Portal where they only see the sections their permissions allow. Staff do NOT need to be merchant owners — their access is strictly controlled by the role you assign.' },
      ...
      { title: 'Add a Staff Member', detail: 'Click "Add Staff". Enter their full name and email address. Choose a role preset: Admin (full access), Manager (all operations), Booking Agent (bookings & scanner only), Scanner (ticket scanning only), or Viewer (read-only, no actions). Click "Add Staff Member" to save.' },
      { title: 'Customize Permissions', detail: 'After selecting a preset, you can fine-tune individual permissions using the toggle switches. There are 9 permission areas: Services (create/edit services), Routes (manage routes), Seating (design seat plans), Timetable (schedule trips), Bookings (view/manage bookings), Discounts (create promos), Scanner (validate tickets), Notifications (send passenger alerts), and Reports (view analytics). Toggle each ON or OFF as needed.' },
      { title: 'Edit a Staff Member', detail: 'Click the pencil icon next to any staff member to change their name, email, role, or permissions. Changes take effect immediately — the staff member will see updated access on their next page load.' },
      { title: 'Activate / Deactivate Staff', detail: 'Use the toggle switch in the Status column to temporarily disable a staff member without deleting them. Deactivated staff cannot log into the travel portal. Reactivate them anytime by toggling back on.' },
      ...
      { title: 'What Staff See When They Log In', detail: 'Staff members log in with their email and password at the normal login page. The system detects their merchant staff role and shows them only the travel sections they have permission for. For example, a "Booking Agent" will see the Bookings page and Scanner, but will not see Routes, Services, or Discounts. The sidebar and navigation automatically hide restricted sections.' },
      { title: 'Role Presets Explained', detail: 'Admin: Full access to everything, equivalent to the merchant owner. Manager: Can manage all operations (services, routes, seating, schedules, bookings, discounts, scanner, notifications, reports). Booking Agent: Can only view/manage bookings and scan tickets — ideal for counter staff. Scanner: Can only scan/validate tickets at boarding gates — ideal for boarding agents. Viewer: No active permissions by default — use this as a starting point for fully custom roles.' },
    ],
    tips: [
      ...
      'You can have unlimited staff members across all roles.',
      'Deactivating a staff member is better than deleting if they might return (e.g., seasonal workers).',
      'Staff permissions are checked in real-time — changes apply immediately without needing to ask staff to log out.',
      'Each staff member must use their own email to log in. Do not share accounts — this ensures accurate audit trails.',
    ],
  },
  {
