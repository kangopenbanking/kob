...
import { Loader2, Search, RotateCcw, CheckCircle2, Clock } from "lucide-react";
...
export default function MerchantRefunds() {
  ...
  const loadData = async () => {
    ...
    if (m) {
      ...
      const { data, count } = await supabase.from("gateway_refunds").select("*", { count: "exact" }).eq("merchant_id", m.id).order("created_at", { ascending: false }).range(from, from + pageSize - 1);
      setRefunds(data || []);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <StatCard title="Total Refunded" value={`${totalAmount.toLocaleString()} XAF`} icon={<RotateCcw className="h-5 w-5" />} />
        <StatCard title="Completed" value={String(completedCount)} icon={<CheckCircle2 className="h-5 w-5" />} />
