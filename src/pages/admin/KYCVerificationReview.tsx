...
import { Shield, FileText, Image, CheckCircle, XCircle, Clock, ExternalLink } from "lucide-react";
...
export default function KYCVerificationReview() {
  ...
  const reviewMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: `KYC submission has been ${reviewAction}.`,
      });
  ...
  const filterByStatus = (status: string) => {
    return kycSubmissions?.filter((kyc) => kyc.status === status) || [];
  };
      ...
      <CardContent className="space-y-4">
        ...
        {kyc.status === "pending" && (
          <div className="flex gap-2 pt-2">
            <Button
              size="sm"
              onClick={() => handleReview(kyc, "approved")}
              className="flex-1"
            >
            ...
            <Button
              size="sm"
              variant="destructive"
              onClick={() => handleReview(kyc, "rejected")}
              className="flex-1"
            >
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Review and approve individual KYC submissions
            </p>
          </div>
        </div>
        ...
        <Tabs defaultValue="pending" className="space-y-4">
          ...
          <TabsContent value="pending" className="space-y-4">
            {isLoading ? (
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {filterByStatus("pending").map((kyc) => (
          ...
          <TabsContent value="approved" className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {filterByStatus("approved").map((kyc) => (
                <KYCCard key={kyc.id} kyc={kyc} />
              ))}
            </div>
          </TabsContent>
          ...
          <TabsContent value="rejected" className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {filterByStatus("rejected").map((kyc) => (
                <KYCCard key={kyc.id} kyc={kyc} />
              ))}
            </div>
          </TabsContent>
            ...
            <DialogHeader>
              ...
              <DialogDescription>
                {reviewAction === "approved"
                  ? "Approve this KYC submission to verify the user's identity."
                  : "Reject this KYC submission if documents are invalid or fraudulent."}
              </DialogDescription>
            </DialogHeader>
