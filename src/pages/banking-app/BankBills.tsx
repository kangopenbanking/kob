...
const billCategories = [
  {
    ...
    providers: [{ id: 'eneo', name: 'ENEO' }, { id: 'aes-sonel', name: 'AES-SONEL' }],
  },
  {
    icon: Droplets, label: 'Water',
    color: 'bg-[hsl(var(--bank-sky))]', fg: 'text-white',
    providers: [{ id: 'camwater', name: 'Camwater' }, { id: 'cde', name: 'CDE' }],
  },
  {
    icon: Wifi, label: 'Internet',
    color: 'bg-[hsl(var(--bank-violet))]', fg: 'text-white',
    providers: [{ id: 'camtel', name: 'Camtel' }, { id: 'mtn-data', name: 'MTN Data' }, { id: 'orange-data', name: 'Orange Data' }],
  },
  {
    icon: Tv, label: 'TV & Cable',
    color: 'bg-[hsl(var(--bank-coral))]', fg: 'text-white',
    providers: [{ id: 'canalplus', name: 'Canal+' }, { id: 'dstv', name: 'DStv' }],
  },
];
...
const BankBills: React.FC = () => {
  ...
  const handlePayBill = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('api-bills', {
        body: { category: selectedCategory.label.toLowerCase(), provider, account_number: meterNumber, amount: Number(amount), currency: 'XAF' },
      });
  ...
  const reset = () => {
    setStep('categories'); setSelectedCategory(null); setProvider(''); setMeterNumber(''); setAmount(''); setPaymentRef('');
  };
    ...
    <div className="flex min-h-screen flex-col px-4 py-6">
      ...
      {step === 'categories' && (
        <div className="flex flex-col gap-3">
          {billCategories.map((cat, i) => {
            const Icon = cat.icon;
            return (
              <motion.button key={cat.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} whileTap={{ scale: 0.98 }}
                onClick={() => handleSelectCategory(cat)} className={`flex items-center gap-4 rounded-2xl ${cat.color} p-5 text-left`}>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex flex-col gap-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={provider} onValueChange={setProvider}>
              ...
              <SelectContent>{selectedCategory.providers.map(p => <SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
      ...
      {step === 'confirm' && selectedCategory && (
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex flex-col gap-4">
          ...
          <Button onClick={() => setShowPin(true)} disabled={paying} className="gap-2" size="lg">
      ...
      <PinConfirmDialog open={showPin} onOpenChange={setShowPin} onConfirmed={handlePayBill} />
    </div>
  );
};
