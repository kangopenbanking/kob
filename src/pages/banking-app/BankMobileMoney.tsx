      ...
      <div className="flex flex-col gap-4">
        <div className="space-y-2">
          ...
          <div className="flex gap-3">
            {providers.map((p) => (
              <motion.button key={p.id} whileTap={{ scale: 0.96 }} onClick={() => setSelectedProvider(p.id)}
                className={`flex-1 rounded-xl border p-3 text-center text-sm font-medium transition-colors ${
                  selectedProvider === p.id ? 'border-primary bg-primary/5 text-primary' : 'bg-card text-foreground'
                }`}
              >
                {p.name}
              </motion.button>
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <Input placeholder="+237 6XX XXX XXX" value={phone} onChange={(e) => setPhone(e.target.value)} className="pl-10" />
          </div>
        </div>

        <div className="space-y-2">
          <Label className="text-sm">Amount (XAF)</Label>
          <Input type="number" placeholder="0" value={amount} onChange={(e) => setAmount(e.target.value)} className="text-xl font-bold text-center h-14" />
        </div>

        <Button onClick={handleSend} disabled={!selectedProvider || !phone || !amount || momoCharge.isPending} className="mt-4 gap-2" size="lg">
