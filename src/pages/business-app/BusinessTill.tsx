...
const BusinessTill: React.FC = () => {
  ...
  return (
    <div className="px-5 md:px-0 pt-2 pb-6">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col">
        ...
        <TabsContent value="till" className="mt-0 flex flex-col space-y-4">
          {/* Search + Scanner */}
          <div className="flex items-center gap-2">
            <div className="relative flex-1">
              <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
              <Input placeholder="Search products..." value={till.searchQuery} onChange={e => till.setSearchQuery(e.target.value)} className="pl-9 h-10 rounded-xl border-border/50 bg-muted/40 text-sm" />
            </div>
            <EnterpriseGate isEnterprise={isEnterprise} onUpgrade={() => toast.info('Upgrade to Enterprise')}>
              <Button variant="outline" size="icon" className="h-10 w-10 shrink-0 rounded-xl" onClick={() => setScannerOpen(true)}>
          ...
          {cartCount > 0 && (
            <Sheet open={cartOpen} onOpenChange={setCartOpen}>
              ...
              <SheetContent side={isMobile ? 'bottom' : 'right'} className={cn(
                isMobile ? 'max-h-[85vh] rounded-t-3xl' : 'w-[420px]',
                'px-4 pb-6',
              )}>
                ...
                <div className="max-h-[30vh] space-y-2 overflow-y-auto">
                  {till.cart.map(item => (
                    <div key={item.variant_id} className="flex items-center gap-2 rounded-xl border border-border/40 bg-card p-2">
