  ...
  const releases = [
    {
      ...
      changes: [
        ...
        { type: "improvement", description: "Sandbox hostname infra closed — worker/wrangler.toml now binds a second [[routes]] block to sandbox-api.kangopenbanking.com/* (same Worker, host-routed). The Worker auto-detects the sandbox host, forces x-kob-environment: sandbox upstream, and re-orders OpenAPI servers[] so sandbox is the default when /openapi.json is fetched from the sandbox host. Setup steps in worker/SANDBOX_HOSTNAME_SETUP.md. Worker bumped to v1.2.0; the only remaining risk from the 2026-04-28 acceptance audit is now closed." },
      ]
    },
    {
    ...
    },
    {
      ...
      changes: [
        ...
        { type: "improvement", description: "Developer Sandbox UI (/developer/sandbox and /developer/sandbox/console) now displays the full credential set with per-field copy buttons and 'shown once' guidance" },
      ]
    },
    {
