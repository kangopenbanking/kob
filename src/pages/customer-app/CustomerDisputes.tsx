...
export default function CustomerDisputes() {
  ...
  const { data: disputes, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!user) return [];
      const { data } = await supabase.from("disputes").select("*").eq("user_id", user.id).order("created_at", { ascending: false });
      return data || [];
    },
  });
    ...
    <div className="min-h-screen bg-background p-4 pb-24 space-y-4">
        ...
        <Dialog open={fileOpen} onOpenChange={setFileOpen}>
          ...
          <DialogContent>
            ...
            <div className="space-y-3">
              ...
              <div><Label>Transaction Reference (optional)</Label><Input value={form.transaction_ref} onChange={e => setForm(p => ({ ...p, transaction_ref: e.target.value }))} placeholder="e.g. TXN-123456" /></div>
      ...
      {isLoading ? (
        ...
        <Card><CardContent className="py-12 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">File a dispute if you have an issue with a transaction.</p>
        </CardContent></Card>
      ) : (
