      ...
      {offers.length === 0 ? (
        <Card className="p-10 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Stay online — new deliveries appear here automatically.
          </p>
        </Card>
      ) : (
        <div className="space-y-3">
          {offers.map((o) => {
            ...
            const expired = ttl === 0;
            return (
              <Card key={o.id + tick} className="p-4 space-y-3 border-primary/40">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-sm font-medium">
                    <MapPin className="size-4 text-primary" />
                    {a.distance_km ? `${Number(a.distance_km).toFixed(1)} km` : "Nearby"} · ETA {a.eta_min ?? 30} min
                  </div>
                  <Badge variant={expired ? "destructive" : "outline"}>{expired ? "Expired" : `${ttl}s`}</Badge>
                </div>
