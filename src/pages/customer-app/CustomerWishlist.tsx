    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-muted/30 to-background flex items-center justify-center px-6">
        <div className="bg-card rounded-3xl p-10 text-center max-w-sm w-full ring-1 ring-border/40 shadow-sm">
          ...
          <h2 className="text-lg font-bold tracking-tight mb-1">{tr('Sign in required')}</h2>
          <p className="text-sm text-muted-foreground mb-5">{tr('Save your favorite shops and products')}</p>
        ...
        <Tabs defaultValue="products" className="w-full mt-5">
          ...
          <TabsContent value="products" className="mt-5">
            {wishlistProducts && wishlistProducts.length > 0 ? (
              ...
              <EmptyCard icon={Package} label={tr('No products saved yet')} cta="Browse marketplace" onClick={() => navigate('/app/stores')} />
            )}
          </TabsContent>
          ...
          <TabsContent value="stores" className="mt-5 space-y-3">
            {favoriteStores && favoriteStores.length > 0 ? (
              ...
              <EmptyCard icon={Store} label={tr('No favorite stores yet')} cta="Discover stores" onClick={() => navigate('/app/stores')} />
            )}
          </TabsContent>
        </Tabs>
      </div>
