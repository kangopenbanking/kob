...
describe("webhook event schema validation", () => {
  ...
  it("rejects missing required nested field", () => {
    const r = validateWebhookEvent({
      ...
      type: "charge.succeeded",
      created: 1,
      data: { object: { id: "chg_x", currency: "XAF", status: "succeeded" } }, // missing amount
    });
    expect(r.ok).toBe(false);
  ...
  it("covers all documented event types with non-empty schemas", () => {
    const expected = ["charge.succeeded", "charge.failed", "payout.completed", "refund.processed", "transfer.completed"];
