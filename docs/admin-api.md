...
curl -H "Authorization: Bearer $TOKEN" \
  "https://wdzkzeahdtxlynetndqw.supabase.co/functions/v1/admin-list-loans?status=approved&limit=10"
```
...
```bash
curl -H "Authorization: Bearer $TOKEN" \
  "https://wdzkzeahdtxlynetndqw.supabase.co/functions/v1/admin-list-savings?status=active&limit=20"
```
...
# AISP consents
curl -H "Authorization: Bearer $TOKEN" \
  "https://wdzkzeahdtxlynetndqw.supabase.co/functions/v1/admin-list-consents?status=Authorised"

# PISP consents
curl -H "Authorization: Bearer $TOKEN" \
  "https://wdzkzeahdtxlynetndqw.supabase.co/functions/v1/admin-list-consents?type=pisp&status=Authorised"
```
