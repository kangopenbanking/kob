...
BEGIN
  ...
  IF NEW.sender_type = 'agent' THEN
    ...
    IF v_conv.guest_email IS NOT NULL AND v_supabase_url IS NOT NULL AND v_service_key IS NOT NULL THEN
  ...
  IF NEW.sender_type = 'guest' AND v_conv.assigned_agent_id IS NOT NULL THEN
    ...
    IF v_agent_user_id IS NOT NULL THEN
      ...
      IF v_agent_email IS NOT NULL AND v_supabase_url IS NOT NULL AND v_service_key IS NOT NULL THEN
        v_payload := jsonb_build_object(
