...
-- Fix calculate_transaction_fee: honor fee_scope precedence (institution → platform)
-- and stop raising when no override exists. Also support merchant-scoped lookups
-- via _institution_id used as the scope key. This unblocks BYO fees and every
-- platform-default fee that was silently failing before.
...
DECLARE
  ...
  v_breakdown JSONB;
BEGIN
  -- Resolve fee structure with precedence: institution-specific → platform default
  ...
  ORDER BY
    -- Prefer institution-specific over platform default
    CASE WHEN fee_scope = 'institution' AND institution_id = _institution_id THEN 0 ELSE 1 END,
    effective_from DESC
  LIMIT 1;

  -- No structure: return zero-fee breakdown instead of raising. Callers (record_transaction_fee)
  -- already skip zero-fee inserts, so this is a no-op for unconfigured types.
  IF NOT FOUND THEN
  ...
  -- Apply max charge cap if configured
  ...
  -- Apply waivers (institution-scoped only)
