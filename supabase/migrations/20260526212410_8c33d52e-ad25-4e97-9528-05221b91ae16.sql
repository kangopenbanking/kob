ALTER TABLE public.pay_by_bank_intents
  ALTER COLUMN merchant_id DROP NOT NULL;

ALTER TABLE public.pay_by_bank_intents
  ADD COLUMN IF NOT EXISTS target_type text NOT NULL DEFAULT 'merchant',
  ADD COLUMN IF NOT EXISTS target_account_id uuid;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'pay_by_bank_intents_target_type_check'
      AND conrelid = 'public.pay_by_bank_intents'::regclass
  ) THEN
    ALTER TABLE public.pay_by_bank_intents
      ADD CONSTRAINT pay_by_bank_intents_target_type_check
      CHECK (target_type IN ('merchant', 'consumer_wallet'));
  END IF;
END $$;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'pay_by_bank_intents_target_required_check'
      AND conrelid = 'public.pay_by_bank_intents'::regclass
  ) THEN
    ALTER TABLE public.pay_by_bank_intents
      ADD CONSTRAINT pay_by_bank_intents_target_required_check
      CHECK (
        (target_type = 'merchant' AND merchant_id IS NOT NULL)
        OR
        (target_type = 'consumer_wallet' AND target_account_id IS NOT NULL)
      );
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_pay_by_bank_intents_target_account
  ON public.pay_by_bank_intents(target_account_id)
  WHERE target_account_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_pay_by_bank_intents_customer_status
  ON public.pay_by_bank_intents(customer_user_id, status);