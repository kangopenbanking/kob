...
paths['/v1/dcr/register'] = {
  post: {
    ...
    security: [{ mtls: [] }],
    parameters: [idempotencyHeader],
    requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['client_name', 'redirect_uris'], properties: { client_name: { type: 'string', example: 'FinTech App Cameroun' }, redirect_uris: { type: 'array', items: { type: 'string' } }, token_endpoint_auth_method: { type: 'string', enum: ['tls_client_auth', 'private_key_jwt', 'client_secret_basic', 'client_secret_post'], default: 'tls_client_auth', description: 'FAPI 1.0 Advanced §5.2.2-2 forbids `none` — unauthenticated client registration is rejected. Public/native clients MUST use PKCE with `private_key_jwt` or `tls_client_auth`.' }, grant_types: { type: 'array', items: { type: 'string' } }, scope: { type: 'string' } } } } } },
...
paths['/v1/standards/iso20022/camt053/parse'] = { post: { tags: ['Standards'], summary: 'Parse camt.053', operationId: 'iso20022Camt053Parser', security: [{ bearerAuth: [] }], requestBody: { required: true, content: { 'application/xml': { schema: { type: 'string' } } } }, responses: { '200': { description: 'Parsed bank statement' }, ...errorResponses }, 'x-iso20022-message': 'camt.053.001.08' } };
paths['/v1/standards/iso20022/camt053/generate'] = { post: { tags: ['Standards'], summary: 'Generate camt.053', description: 'Generate an ISO 20022 camt.053.001.08 Bank-to-Customer Statement message for end-of-day reconciliation. Bank integrators use this for intraday and EOD reconciliation workflows.', operationId: 'iso20022Camt053Generator', security: [{ bearerAuth: [] }], parameters: [idempotencyHeader], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['account_id', 'from_datetime', 'to_datetime'], properties: { account_id: { type: 'string', description: 'Account identifier whose transactions are reported.' }, from_datetime: { type: 'string', format: 'date-time' }, to_datetime: { type: 'string', format: 'date-time' }, currency: { type: 'string', example: 'XAF' }, message_id: { type: 'string', description: 'Optional message identifier; auto-generated when omitted.' } } } } } }, responses: { '200': { description: 'Generated camt.053 statement (XML)', content: { 'application/xml': { schema: { type: 'string' } } } }, ...errorResponses }, 'x-iso20022-message': 'camt.053.001.08' } };
...
serve(async (req) => {
  ...
  try {
    const openapiSpec = {
      ...
      servers: [
        { url: Deno.env.get('PUBLIC_API_BASE_URL') ?? 'https://api.kangopenbanking.com/v1', description: 'Production' },
        { url: (Deno.env.get('PUBLIC_API_BASE_URL') ?? 'https://api.kangopenbanking.com/v1').replace('://api.', '://sandbox-api.'), description: 'Sandbox' },
      ],
      security: [{ bearerAuth: [] }],
      tags: [
      ...
      components: {
        securitySchemes: {
          ...
          oauth2: {
            ...
            flows: {
              authorizationCode: {
                authorizationUrl: `${Deno.env.get('PUBLIC_API_BASE_URL') ?? 'https://api.kangopenbanking.com/v1'}/oauth/authorize`,
                tokenUrl: `${Deno.env.get('PUBLIC_API_BASE_URL') ?? 'https://api.kangopenbanking.com/v1'}/oauth/token`,
                scopes: {
