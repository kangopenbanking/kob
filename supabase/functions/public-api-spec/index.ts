...
paths['/v1/settlement/calculate'] = {
  post: { tags: ['Settlement'], summary: 'Calculate settlement', operationId: 'settlementCalculate', security: [{ bearerAuth: [] }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['institution_id', 'period_start', 'period_end'], properties: { institution_id: { type: 'string' }, period_start: { type: 'string', format: 'date-time' }, period_end: { type: 'string', format: 'date-time' } } } } } }, responses: { '200': { description: 'Settlement calculation' }, ...errorResponses } },
};

paths['/v1/settlement/process'] = {
  post: { tags: ['Settlement'], summary: 'Process settlement', operationId: 'settlementProcess', security: [{ bearerAuth: [] }], parameters: [idempotencyHeader], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object' } } } }, responses: { '200': { description: 'Settlement processed' }, ...errorResponses } },
};
...
paths['/v1/banking/facilitated-transfer'] = {
  post: { tags: ['Banking Operations'], summary: 'Institution-facilitated bank transfer', description: 'Initiate a bank payout facilitated by a financial institution through the Flutterwave rail. Includes KOB fee calculation and settlement tracking. The institution_id determines fee structure and settlement bucket.', operationId: 'bankingFacilitatedTransfer', security: [{ bearerAuth: [] }], parameters: [idempotencyHeader], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['account_number', 'bank_code', 'amount', 'institution_id'], properties: { account_number: { type: 'string', example: '1234567890' }, bank_code: { type: 'string', example: 'SGCM' }, bank_name: { type: 'string', example: 'Société Générale Cameroun' }, amount: { type: 'number', example: 100000 }, currency: { type: 'string', default: 'XAF' }, narration: { type: 'string', example: 'Salary payment' }, institution_id: { type: 'string', format: 'uuid', description: 'Facilitating institution' }, account_name: { type: 'string', example: 'Jean Dupont' } } } } } }, responses: { '200': { description: 'Facilitated transfer initiated', content: { 'application/json': { schema: { type: 'object', properties: { id: { type: 'string', format: 'uuid' }, transaction_ref: { type: 'string' }, amount: { type: 'number' }, kob_fee_amount: { type: 'number' }, currency: { type: 'string' }, status: { type: 'string', enum: ['pending', 'completed', 'failed'] }, bank_name: { type: 'string' }, account_number: { type: 'string' }, flutterwave_ref: { type: 'string' }, created_at: { type: 'string', format: 'date-time' } } } } } }, ...errorResponses } },
};
