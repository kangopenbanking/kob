...
serve(async (req) => {
  ...
  try {
    ...
    const { merchant_id, name, channel = 'mobile_money', account_number, bank_code, bank_name, phone, email, metadata } = body;
    ...
    const { data: beneficiary, error } = await supabase.from('gateway_beneficiaries').insert({
      merchant_id, name, channel, account_number, bank_code, bank_name, phone, email, metadata: metadata || {},
    }).select().single();
