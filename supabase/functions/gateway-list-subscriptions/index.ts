...
serve(async (req) => {
  ...
  try {
    ...
    let query = supabase
      ...
      .select('*, gateway_payment_plans(*)', { count: 'exact' })
    ...
    return new Response(JSON.stringify({ data: subs, total: count, limit, offset }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (err) {
    const errorId = crypto.randomUUID().slice(0, 8);
    console.error(`[${errorId}] list-subscriptions error:`, err);
