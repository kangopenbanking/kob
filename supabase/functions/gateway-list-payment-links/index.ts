...
serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ data: links, total: count, limit, offset }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (err) {
    const errorId = crypto.randomUUID().slice(0, 8);
    console.error(`[${errorId}] list-payment-links error:`, err);
