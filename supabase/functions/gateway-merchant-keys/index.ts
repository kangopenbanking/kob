...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'rotate') {
      ...
      if (env === 'live') {
        if (merchant.kyb_status !== 'approved') return problem(403, 'KYB Required', 'KYB must be approved before rotating production keys.');
        if (!merchant.live_mode_enabled) return problem(403, 'Live Mode Disabled', 'Enable Go Live mode before rotating production keys.');
      }
      const prefix = env === 'live' ? 'pk_live_' : 'pk_test_';
      const secretPrefix = env === 'live' ? 'sk_live_' : 'sk_test_';
      const publicKey = prefix + crypto.randomUUID().replace(/-/g, '');
